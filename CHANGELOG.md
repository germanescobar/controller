...
- **Claude picker: add Claude Opus 5.5**. Anthropic released Opus 5.5 on September 22, 2026 (the first model in the Claude 5.5 series), and `getClaudeModels()` in `server/lib/models.ts` now lists `claude-opus-5-5` (Opus 5.5) as the new `default` flagship. `claude-opus-5` (Opus 5) is kept as a non-default entry so users with sessions pinned to it keep a target, alongside the existing `claude-sonnet-5` and `claude-fable-5`. The model id follows the same hyphenated convention the project already uses for the Claude CLI's `--model` flag (e.g. `claude-opus-4-8` in older fixtures).
...
- **UI: parent + children rows in the floating focus panel (issue #384)**. Coordinators (issue #351 + #363) had no in-view signal that a session has a parent or children — the only hint was the sidebar's parent-subtree tree, which forced a sidebar detour. The floating `FocusConversationControls` panel now grows a stacked `Parent` row (when the current session has one) and a `Children` row (when it has any) below the existing Add to radar / Done / Next row, matching the vertical style of the other panel rows. Each title is a truncated, clickable anchor whose `data-controller-uri="controller://project/<pid>/worktree/<wid>/session/<sid>"` defers to the existing `controller://` link resolver — no new IPC for navigation itself. The existing `GET /api/sessions/:sessionId/children` route (issue #353) is extended to return `parentSession` + `parentProjectId` + `parent` (when the current session has a real parent) and `null` for all three when it doesn't, so the panel can build a working anchor without falling back to the URL param. Live updates flow through the existing project-events stream — `App.tsx` calls `bumpRelationshipsRefresh()` inside the same debounced handler that bumps `eventsRefreshKey`, so a child spawned or finished in another tab is reflected on the next event without polling. A new in-memory store (`client/src/lib/session-relationships.ts`) plus a `useSessionRelationships(sessionId)` hook keeps multiple panel mounts in sync; `SessionView` keys the hook off `activeStreamSessionId ?? sessionId` so the rows stay correct while a brand-new session is attaching from a `startSession` call. Nine new panel tests cover the conditional rendering and the click-to-navigate wiring, including the "no parent + no children" unchanged-render contract, the missing-`projectId` fallback (skips the anchor rather than emitting a malformed URI), long-title truncation, and the mobile out-of-scope guard (the multi-line rows are desktop-only; mobile keeps its existing compact strip). The server's `listChildSessions` and the strict-archive rule are untouched — the per-child project walk happens at the route layer so the cheaper existing helper keeps its current shape.
- **UI: branch-this-conversation icon below every assistant turn (#364, follow-up)**. The server-side `POST /sessions/branch` verb shipped in #365 had no UI surface, so the only way to branch was the `controller sessions branch` CLI. A `GitFork` icon now sits beside the existing copy icon under each assistant message (both the persisted timeline and the live stream). Clicking it calls `branchSession(projectId, sourceSessionId)` — the route always spawns an agent at branch time so the new session id is a real provider thread id, even when the user clicks from the UI without typing a message (PR review P1 from chatgpt-codex-connector on #381: a Controller-chosen UUID with no provider backing would fail the user's first follow-up turn because the provider's `--resume` / `thread/resume` would target a nonexistent thread). The user lands on the new session's empty composer with the source conversation in context; the agent has already initialized and is ready for the first real turn. The composer pickers (agent / model / mode / reasoning / fast mode) stay unlocked on the branched session until the user types their first turn (PR review P2 from chatgpt-codex-connector on #381) — a new `unstarted` flag on the session file signals the picker gate to keep them open; the flag is cleared by the server's `persistSessionStart` on the first follow-up resume. The CLI verb is unchanged — `branch <src> "msg"` still requires a non-empty message because the CLI is the caller that's expected to start the first turn. The branch icon shows a spinner during the request and is disabled to prevent double-fires. The route's `seedBranchFromSource` is reused for the on-disk shape, so a UI-branched session is byte-identical to an agent-spawned one (same source-derived defaults, same `[/branch: source->target]` marker on the events file, same `parentId` linkage).
