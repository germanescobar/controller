...
// Route registration order matters: Express matches the first
// registration that fits the path, so the fixed `/memory/:scope/pinned`
// and `/memory/:scope/search` endpoints have to be declared *before* the
// catch-all `/memory/:scope/:slug` route, otherwise a slug literally
// named `pinned` or `search` shadows them. (PR review, P1.)
...
memoryRouter.delete("/memory/:scope/:slug", async (req: Request, res: Response) => {
  ...
  // PR review (P2): validate the projectId against the project registry
  // before constructing the path. Without this, a crafted `projectId`
  // such as `../global` is normalized by `path.join` to the global
  // scope and `deleteMemory` would happily remove a global note,
  // violating scope isolation. The other routes already do this check;
  // DELETE should too.
  if (scope === "project" && projectId) {
    const project = await getProject(projectId);
    if (!project) {
