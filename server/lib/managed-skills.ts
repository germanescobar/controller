...
export function managedMarker(dirName: string): string {
  return `<!-- managed-by: controller (${dirName}) -->`;
}
...
must be made at the project/source path (\`$SOURCE_PATH/.controller/\`),
...
prefer native \`.controller\` scripts.
...
\`\`\`bash
cat > .env.local <<EOF
# Generated by .controller/setup.sh for this worktree.
PORT=\$API_PORT
API_PORT=\$API_PORT
...
- Scripts live at \`$SOURCE_PATH/.controller/setup.sh\` and
  \`$SOURCE_PATH/.controller/run.sh\`. Controller copies them into each
  worktree on creation.
- Make scripts executable:
  \`chmod +x $SOURCE_PATH/.controller/setup.sh $SOURCE_PATH/.controller/run.sh\`.
- \`setup.sh\` and \`run.sh\` run from the worktree root (\`\$WORKTREE_PATH\`).

## Simple project template

For a Node/Vite + API project, in \`$SOURCE_PATH/.controller/\`:
...
1. Read the current scripts at \`$SOURCE_PATH/.controller/\` and the
...
4. Validate bash syntax:
   \`bash -n $SOURCE_PATH/.controller/setup.sh\` and
   \`bash -n $SOURCE_PATH/.controller/run.sh\`.
...
If \`.controller/setup.sh\` or \`run.sh\` are missing, Controller reads,
in order:
...
\`$SOURCE_PATH/.controller/\` and delete the fallback config files.
`;
...
- \`worktrees create\` runs the project's \`.controller/setup.sh\`
