...
/**
 * Document the cross-session discoverability surface (issue #353) and
 * the coordinator primitives built on top of it (issue #351).
 *
 ...
 * `provider` matches the agent that was just spawned), (b) find its
 * own children via `controller sessions list --parent self` or the
 * dedicated `sessions children` verb, and (c) post a message to a
 * child via `sessions send` without spawning a fresh session.
 */
function sessionsListIntro(): string | null {
  ...
  return [
    ...
    `  - \`${cli} sessions send <targetSessionId> <message> --from <id|self>\``,
    `    enqueues a durable user turn on the target session whose text is`,
    `    the canonical \`[/from: <parentTitle>] <controller-uri> <message>\` marker. The`,
    `    child session runs the message after its current turn (or immediately`,
    `    when idle); the literal \`controller://\` URI links back to the`,
    `    parent conversation. Use this when a coordinator wants to`,
    ...
    `  - \`${cli} sessions children <parentId|self>\` lists the children of`,
    ...
    `  - \`${cli} sessions branch <sourceSessionId|self> <message>\``,
    `    \`[--provider|--agent <id>] [--model <model>]\``,
    `    \`[--mode default|plan] [--title <title>] [--from <id|self>]\``,
    `    forks an existing conversation into a brand-new session whose`,
    `    transcript is a copy of the source's transcript (issue #364).`,
    `    The first turn of the new branch runs on the requested`,
    `    provider/model/mode (or source's defaults when omitted); later`,
    `    turns use whatever the user picks in the composer. The`,
    `    branch marker (\`[/branch: codex->claude/opus-5] …\`) is recorded`,
    `    as the first message so the chat view audits which agent/model`,
    `    ran the first turn. \`<sourceSessionId>\` accepts \`self\` (via`,
    `    \`$CONTROLLER_SESSION_ID\`) so an agent can branch its own`,
    `    conversation: \`branch self "Review the design" --agent claude`,
    `    --model opus-5\` runs the next turn on a different agent while`,
    `    keeping the source's transcript visible in the chat. The new`,
    `    session is recorded as a child of the source (\`sessions list`,
    `    --parent <sourceId>\` and \`sessions children <sourceId>\` both`,
    `    surface it). Use \`branch\` when you want a stronger or cheaper`,
    `    model for a specific question mid-conversation without losing`,
    `    the prior transcript.`,
    `  - \`${cli} sessions monitor start <sessionId> --description <text>\``,
    ...
    `   ${cli} sessions send "$CHILD" "look at issue 190 first" --from "$SELF"`,
    ...
    `   ${cli} sessions monitor start "$CHILD" \\`,
    `     --description "CI" \\`,
    `     --command "gh pr checks --watch" \\`,
    `     --on-line '^\\[CI\\] (passed|failed)$'`,
    `   # 5. Later, see your children.`,
    `   ${cli} sessions children self`,
  ].join("\n");
}
